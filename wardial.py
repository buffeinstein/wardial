...
async def _wardial_async(hosts, max_connections=500, timeout=10, schema='http'):
    ...
    async with aiohttp.ClientSession(
        ...
        servers = []
        for host in hosts:
           servers.append(is_server_at_host(session, host))
...
def wardial(hosts, **kwargs):
    ...
    running = loop.run_until_complete(_wardial_async(hosts, **kwargs))
    loop.close()

    return [host for host, result in zip(hosts, running) if result]
